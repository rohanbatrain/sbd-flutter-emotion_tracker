...
class AdUnitIds {
  ...
  static const String rewardedAdUnitId =
      'ca-app-pub-2845453539708646/2553416618'; // Updated to production ID
  static const String adaptiveBannerAdUnitId =
...
// Ad loading states
enum AdLoadingState { notLoaded, loading, loaded, failed }
...
class AdNotifier extends StateNotifier<AdState> {
  ...
  Future<void> _initializeAdMob() async {
    // Only initialize on mobile platforms (iOS/Android). Desktop implementations
    // of google_mobile_ads may not exist which causes MissingPluginException.
    if (!_isMobile) return;
    try {
  ...
  Future<void> loadBannerAd(String adId, {AdSize size = AdSize.banner}) async {
    ...
    final newBannerAdStates = Map<String, AdLoadingState>.from(
      state.bannerAdStates,
    );
  ...
  Future<void> loadRewardedAdWithCustomId({
    ...
    await RewardedAd.load(
      ...
      rewardedAdLoadCallback: RewardedAdLoadCallback(
        onAdLoaded: (ad) {
          print(
            '[AdProvider] Loaded rewarded ad with adUnitId: $adUnitId for user: $userId',
          );
          ad.setServerSideOptions(
        ...
        onAdFailedToLoad: (error) {
          print(
            '[AdProvider] Failed to load rewarded ad with adUnitId: $adUnitId for user: $userId, error: $error',
          );
          state = state.copyWith(rewardedAdState: AdLoadingState.failed);
        },
      ),
  ...
  Future<void> showRewardedAd({
    ...
    await state.rewardedAd!.show(
      onUserEarnedReward: (Ad ad, RewardItem reward) {
        print(
          '[AdProvider] showRewardedAd: onUserEarnedReward called. Username: $username, Reward: ${reward.amount} ${reward.type}',
        );
        if (onRewardCallback != null &&
            username != null &&
            username.isNotEmpty) {
          print(
            '[AdProvider] showRewardedAd: onRewardCallback triggered for username: $username',
          );
...
final bannerAdStateProvider = Provider.family<AdLoadingState, String>((
  ref,
  adId,
) {
